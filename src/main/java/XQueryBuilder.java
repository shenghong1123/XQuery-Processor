...
public class XQueryBuilder extends XQueryBaseVisitor<MyNodeList> {
    ...
    public MyNodeList visitJoinClause(@NotNull XQueryParser.JoinClauseContext ctx) {
        ...
        System.err.println("First Length" + result1.getLength());
        MyNodeList result2 = visit(ctx.xq(1)); // get the tuple list from the second partition
        System.err.println("Second Length" + result2.getLength());
        MyNodeList result = new MyNodeList(); // initialize result list

        List<TerminalNode> att1 = ctx.attlist(0) != null? ctx.attlist(0).TAGNAME(): new ArrayList<>();
        List<TerminalNode> att2 = ctx.attlist(1) != null? ctx.attlist(1).TAGNAME(): new ArrayList<>();

        if(att1.size() == 0 && att2.size() == 0) { // In case of empty attribute list
            System.out.println("Cartesian product happens");
            for(Node m: result1) {
                for(Node n: result2) {
                    MyNodeList content1 = XMLBasicOperator.getAllChildren(new MyNodeList(m));
                    MyNodeList content2 = XMLBasicOperator.getAllChildren(new MyNodeList(n));
                    MyNodeList concat = XMLBasicOperator.concat(content1, content2);
                    Node tuple = XMLBasicOperator.makeElement(concat, "tuple").item(0);
                    result.add(tuple);
                }
            }
            ctxMap.put(ctx, result);
        ...
        for(Node n: result1) {
            ...
            for(TerminalNode t: att1) { // build key list
                String s = t.getText();
                Node cur = XMLBasicOperator.getChildrenByTag(new MyNodeList(n), s).item(0);
                //System.err.println(cur.getTextContent());
                key += cur.getTextContent();
            }
            //for(Node m: key) System.out.println(m.getFirstChild().getNodeValue());
            if(joinMap.containsKey(key)) joinMap.get(key).add(n);
            else joinMap.put(key, new MyNodeList(n));
        }

        for(Node n: result2) {
            String key = new String();
            for(TerminalNode t: att2) { // build probe list
                String s = t.getText();
                Node cur = XMLBasicOperator.getChildrenByTag(new MyNodeList(n), s).item(0);
                key += cur.getTextContent();
            }
        ...
        System.err.println("Join result length" + result.getLength());
        ctxMap.put(ctx, result);
        return result;
    }
    ...
    public MyNodeList visitValueEqualCondition(@NotNull XQueryParser.ValueEqualConditionContext ctx) {
        ...
        if(ctx.xq(1) instanceof XQueryParser.StringConstantContext) {
            ...
            for(Node n: leftResult) {
                if(n.getTextContent().equals(content)) {
